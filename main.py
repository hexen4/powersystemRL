...
def test(n_runs, start, test_length, pv_profile, wt_profile, load_profile, price_profile, run, sequence_model_type='rnn', log=False, log_path=None): # TODO need to chang from ONLY actual costs -> all variables
    assert(start >= 0 and start < pv_profile.shape[0])
    ...
    for _ in range(n_runs):
        ts.run_timeseries(net, time_steps=time_steps, verbose=False, continue_on_divergence=False) #related to pandapower
