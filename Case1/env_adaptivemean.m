classdef env_adaptivemean < rl.env.MATLABEnvironment    
    properties
        ...
        f1;
        minprice;
        obsMu;           % Running mean of rewards
        obsVar;          % Running variance (EMA)
        obsInitialized;  % Flag for initialization
    ...
    methods              
        ...
        function [Observation, Reward, IsDone] = step(this, Action)
            ...
            % Update time and compute the new raw state (unscaled)
            this.time = this.time + 1;
            rawObservation = this.update_state(this.State, Action, this.time);
        
            % Compute the reward based on raw (unscaled) state values
            [Reward, logStruct, ~] = this.calculate_reward(Action, rawObservation, this.time, this.State);
            
            % ---- Normalize the raw observation for the agent's input ----
            % Here, we use a simple z-score normalizer as an example.
            % You can maintain running statistics (obsMu and obsVar) as before.
            if ~this.obsInitialized
                this.obsMu = rawObservation;                  % Initialize mean with the first observation
                this.obsVar = ones(size(rawObservation)) * 1e-4; % Start with a small positive variance
                this.obsInitialized = true;
            else
                % Update running mean and variance (element-wise)
                this.obsMu = this.alphaReward .* rawObservation + (1 - this.alphaReward) .* this.obsMu;
                diff = rawObservation - this.obsMu;
                this.obsVar = this.alphaReward .* (diff.^2) + (1 - this.alphaReward) .* this.obsVar;
            end
            sigma = sqrt(this.obsVar) + 1e-8;
            normObservation = (rawObservation - this.obsMu) ./ sigma;
            % Optionally, you can squash normObservation using tanh if desired:
            % normObservation = tanh(normObservation);
            
            % Update the internal state with the raw observation (so dynamics and reward remain unscaled)
            this.State = rawObservation;
            
            % Save logs and check termination as usual
            this.EpisodeLogs{end+1} = logStruct;
            IsDone = (this.time >= this.H);
            if IsDone && (this.training == 1)
                this.reset();
            end
        
            % Return the normalized observation for training the agent
            Observation = normObservation;
        end
    ...
    methods     
        ...
        function [reward,logStruct, Observation_updated] = calculate_reward(this, scaled_action, next_state, time, state)
            ...
            %% interval optimisation
            consumer_benefit_limit = max(-5,time^1*consumer_benefit_penalty); %clip at -30 for maximum benefit and minimise 
